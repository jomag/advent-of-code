...
with open("input.txt", "r") as f:
    part_two_input = [line.strip() for line in f.readlines()]

part_two_input[39] = part_two_input[39][:39] + "@#@" + part_two_input[39][42:]
part_two_input[40] = part_two_input[40][:39] + "###" + part_two_input[40][42:]
part_two_input[41] = part_two_input[41][:39] + "@#@" + part_two_input[41][42:]

part_two_inputs = [
    [line[:41] for line in part_two_input[:41]],
    [line[40:] for line in part_two_input[:41]],
    [line[:41] for line in part_two_input[40:]],
    [line[40:] for line in part_two_input[40:]],
]

total = 0
for inp in part_two_inputs:
    keys = [ch for line in inp for ch in line if is_key(ch)]
    doors = [ch for line in inp for ch in line if is_door(ch)]
    for door in doors:
        if door.lower() not in keys:
            inp = [line.replace(door, ".") for line in inp]
    n = solve(inp)
    print("Partial: ", n)
    total += n


print("Final part two: ", total)
